<?php
class Service_case_payment_method_model extends CI_Model {
  ...
  public function get_one($s_num) {
    ...
    $tbl_service_case_payment_method = $this->zi_init->chk_tbl_no_lang('service_case_payment_method'); // 開結案服務-繳費方式
    $tbl_account = $this->zi_init->chk_tbl_no_lang('sys_account'); // 登入帳號
    $s_num = (int)$this->db->escape_like_str($s_num);
    $row = NULL;
    $sql = "select {$tbl_service_case_payment_method}.*,
                   sys_acc.acc_name as b_acc_name,
                   sys_acc2.acc_name as e_acc_name,
                   case {$tbl_service_case_payment_method}.is_available
                     when '0' then '停用'
                     when '1' then '啟用'
                   end as is_available_str,
                   count({$tbl_service_case}.sec09) as now_use_cnt
            from {$tbl_service_case_payment_method}
            left join {$tbl_account} sys_acc on sys_acc.s_num = {$tbl_service_case_payment_method}.b_empno
            left join {$tbl_account} sys_acc2 on sys_acc2.s_num = {$tbl_service_case_payment_method}.e_empno
            left join {$tbl_service_case} on {$tbl_service_case}.sec09 = {$tbl_service_case_payment_method}.s_num
            where {$tbl_service_case_payment_method}.d_date is null
                  and {$tbl_service_case_payment_method}.s_num = ?
                  and {$tbl_service_case}.d_date is null
            group by {$tbl_service_case_payment_method}.s_num
            order by {$tbl_service_case_payment_method}.s_num desc
           ";
    //u_var_dump($sql);
  ...
  public function chk_duplicate($fd_name) {
    ...
    $sql = "select {$tbl_service_case_payment_method}.*
            ...
            from {$tbl_service_case_payment_method}
            where {$tbl_service_case_payment_method}.d_date is null
                  and {$tbl_service_case_payment_method}.fd_name = ?
            order by {$tbl_service_case_payment_method}.s_num desc
           ";
    //u_var_dump($sql);
  ...
  public function get_all_is_available() {
    ...
    $sql = "select {$tbl_service_case_payment_method}.*
            ...
            from {$tbl_service_case_payment_method}
            where {$tbl_service_case_payment_method}.d_date is null
                  and {$tbl_service_case_payment_method}.is_available = 1 /* 啟用 */
            order by {$tbl_service_case_payment_method}.s_num asc
           ";
    //u_var_dump($sql);
    $rs = $this->db->query($sql, array($sql));
    if($rs->num_rows() > 0) { // 有資料才執行
  ...
  public function get_que($q_str=NULL,$pg=NULL) {
    ...
    $where = " {$tbl_service_case_payment_method}.d_date is null 
               and {$tbl_service_case}.d_date is null
             ";
    $order = " {$tbl_service_case_payment_method}.s_num asc ";
    $get_data = $this->input->get();
    if(!empty($get_data['que_str'])) { // 全文檢索
      $que_str = $get_data['que_str'];
      $where .= " and ({$tbl_service_case_payment_method}.sep01 like '%{$que_str}%' /* 繳費方式名稱 */
                      )
                ";
    }

    if(!empty($get_data['que_sep01'])) { // 繳費方式名稱
      $que_sep01 = $get_data['que_sep01'];
      $que_sep01 = $this->db->escape_like_str($que_sep01);
      $where .= " and {$tbl_service_case_payment_method}.sep01 like '%{$que_sep01}%'  /* 繳費方式名稱 */ ";
    }
    ...
    // 計算查詢條件後的總筆數
    $sql_cnt = "select {$tbl_service_case_payment_method}.s_num
                from {$tbl_service_case_payment_method}
                left join {$tbl_service_case} on {$tbl_service_case}.sec09 = {$tbl_service_case_payment_method}.s_num
                where $where
                group by {$tbl_service_case_payment_method}.s_num
                order by {$tbl_service_case_payment_method}.s_num
               ";
    //u_var_dump($sql_cnt);
  ...
  public function save_add() {
    ...
    if(!$this->db->insert($tbl_service_case_payment_method, $post_data)) {
  ...
  public function save_upd() {
    ...
    if(!$this->db->update($tbl_service_case_payment_method, $post_data)) {
  ...
  public function save_is_available() {
    ...
    if(!$this->db->update($tbl_service_case_payment_method, $data)) {
      $rtn_msg = $this->lang->line('upd_ng'); // 修改失敗!!
    }
    echo $rtn_msg;
    return;
  ...
  private function _aes_fd() {
    ...
    foreach ($this->aes_fd as $k_fd_name => $v_fd_name) {
      $aes_fd .= ",AES_DECRYPT({$tbl_service_case_payment_method}.{$v_fd_name},'{$this->db_crypt_key2}') as {$v_fd_name}\n";
    }
    return($aes_fd);
  }
  // **************************************************************************
