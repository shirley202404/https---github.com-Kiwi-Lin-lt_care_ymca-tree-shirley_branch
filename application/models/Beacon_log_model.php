<?php
class Beacon_log_model extends CI_Model {
  ...
  public function get_one($s_num) {
    $tbl_beacon_log = $this->zi_init->chk_tbl_no_lang('beacon_log'); // Beacon Log 資料
    $tbl_account = $this->zi_init->chk_tbl_no_lang('sys_account'); // 登入帳號
    $s_num = (int)$this->db->escape_like_str($s_num);
    $row = NULL;
    $sql = "select {$tbl_beacon_log}.*,
                   sys_acc.acc_name as b_acc_name,
                   sys_acc2.acc_name as e_acc_name,
                   case {$tbl_beacon_log}.bnl02
                     when '1' then '簽到'
                     when '2' then '簽退'
                   end as {$tbl_beacon_log}.bnl02_str,
                   case {$tbl_beacon_log}.is_available
                     when '0' then '停用'
                     when '1' then '啟用'
                   end as is_available_str
                   
            from {$tbl_beacon_log}
            left join {$tbl_account} sys_acc on sys_acc.s_num = {$tbl_beacon_log}.b_empno
            left join {$tbl_account} sys_acc2 on sys_acc2.s_num = {$tbl_beacon_log}.e_empno
            where {$tbl_beacon_log}.d_date is null
                  and {$tbl_beacon_log}.s_num = ?
            order by {$tbl_beacon_log}.s_num desc
           ";
    //u_var_dump($sql);
  ...
  public function get_all_is_available() {
    ...
    $sql = "select {$tbl_beacon_log}.*
            ...
            from {$tbl_beacon_log}
            where {$tbl_beacon_log}.d_date is null
                  and {$tbl_beacon_log}.is_available = 1 /* 啟用 */
            order by {$tbl_beacon_log}.s_num desc
           ";
    //u_var_dump($sql);
    $rs = $this->db->query($sql, array($sql));
  ...
  public function get_que($q_str=NULL,$pg=NULL) {
    $tbl_beacon_log = $this->zi_init->chk_tbl_no_lang('beacon_log'); // Beacon Log 資料
    $where = " {$tbl_beacon_log}.d_date is null ";
    $order = " {$tbl_beacon_log}.s_num desc ";
    $get_data = $this->input->get();
    if(!empty($get_data['que_str'])) { // 全文檢索
      $que_str = $get_data['que_str'];
      $where .= " and ({$tbl_beacon_log}.bnl01_uuid like '%{$que_str}%' /* Beacon(UUID) */                       
                       or {$tbl_beacon_log}.bnl02 like '%{$que_str}%' /* 打卡類型 1=簽到，2=簽退 */
                      )
                ";
    }

    if(!empty($get_data['que_bnl01_uuid'])) { // Beacon(UUID)
      $que_bnl01_uuid = $get_data['que_bnl01_uuid'];
      $que_bnl01_uuid = $this->db->escape_like_str($que_bnl01_uuid);
      $where .= " and {$tbl_beacon_log}.bnl01_uuid = '{$que_bnl01_uuid}'  /* Beacon(UUID) */ ";
    }
    if(!empty($get_data['que_bnl02'])) { // 打卡類型 1=簽到，2=簽退
      $que_bnl02 = $get_data['que_bnl02'];
      $que_bnl02 = $this->db->escape_like_str($que_bnl02);
      $where .= " and {$tbl_beacon_log}.bnl02 = '{$que_bnl02}'  /* 打卡類型 1=簽到，2=簽退 */ ";
    }
    ...
    // 計算查詢條件後的總筆數
    $sql_cnt = "select {$tbl_beacon_log}.s_num
                from {$tbl_beacon_log}
                where $where
                group by {$tbl_beacon_log}.s_num
                order by {$tbl_beacon_log}.s_num
               ";
    //u_var_dump($sql_cnt);
  ...
  public function save_add() {
    ...
    if(!$this->db->insert($tbl_beacon_log, $data)) {
  ...
  public function save_upd() {
    ...
    if(!$this->db->update($tbl_beacon_log, $data)) {
  ...
  private function _aes_fd() {
    ...
    foreach ($this->aes_fd as $k_fd_name => $v_fd_name) {
      $aes_fd .= ",AES_DECRYPT({$tbl_beacon_log}.{$v_fd_name},'{$this->db_crypt_key2}') as {$v_fd_name}\n";
    }
    return($aes_fd);
  }
  // **************************************************************************
