<?php
class Meal_replacement_day_set_model extends CI_Model {
  ...
  public function get_one($s_num) {
    $tbl_meal_replacement_day_set = $this->zi_init->chk_tbl_no_lang('meal_replacement_day_set'); // 代餐送餐時間設定
    $tbl_account = $this->zi_init->chk_tbl_no_lang('sys_account'); // 登入帳號
    $s_num = (int)$this->db->escape_like_str($s_num);
    $row = NULL;
    $sql = "select {$tbl_meal_replacement_day_set}.*,
                   sys_acc.acc_name as b_acc_name,
                   sys_acc2.acc_name as e_acc_name,
                   case {$tbl_meal_replacement_day_set}.is_available
                     when '0' then '停用'
                     when '1' then '啟用'
                   end as is_available_str
                   
                   
            from {$tbl_meal_replacement_day_set}
            left join {$tbl_account} sys_acc on sys_acc.s_num = {$tbl_meal_replacement_day_set}.b_empno
            left join {$tbl_account} sys_acc2 on sys_acc2.s_num = {$tbl_meal_replacement_day_set}.e_empno
            where {$tbl_meal_replacement_day_set}.d_date is null
                  and {$tbl_meal_replacement_day_set}.s_num = ?
            order by {$tbl_meal_replacement_day_set}.s_num desc
           ";
    //u_var_dump($sql);
  ...
  public function chk_duplicate($fd_name) {
    ...
    $sql = "select {$tbl_meal_replacement_day_set}.*
            ...
            from {$tbl_meal_replacement_day_set}
            where {$tbl_meal_replacement_day_set}.d_date is null
                  and {$tbl_meal_replacement_day_set}.fd_name = ?
            order by {$tbl_meal_replacement_day_set}.s_num desc
           ";
    //u_var_dump($sql);
  ...
  public function get_all_is_available() {
    ...
    $sql = "select {$tbl_meal_replacement_day_set}.*
            ...
            from {$tbl_meal_replacement_day_set}
            where {$tbl_meal_replacement_day_set}.d_date is null
                  and {$tbl_meal_replacement_day_set}.is_available = 1 /* 啟用 */
            order by {$tbl_meal_replacement_day_set}.s_num desc
           ";
    //u_var_dump($sql);
    $rs = $this->db->query($sql, array($sql));
  ...
  public function get_que($q_str=NULL,$pg=NULL) {
    $tbl_meal_replacement_day_set = $this->zi_init->chk_tbl_no_lang('meal_replacement_day_set'); // 代餐送餐時間設定
    $where = " {$tbl_meal_replacement_day_set}.d_date is null ";
    $order = " {$tbl_meal_replacement_day_set}.s_num desc ";
    $get_data = $this->input->get();
    if(!empty($get_data['que_str'])) { // 全文檢索
      $que_str = $get_data['que_str'];
      $where .= " and ({$tbl_meal_replacement_day_set}.mlds01 like '%{$que_str}%' /* 送餐時間調整-起 */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds02 like BINARY '%{$que_str}%' /* 送餐時間調整-迄 */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds03_begin like BINARY '%{$que_str}%' /* 中餐送餐日-起-MEMO(非熟代) */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds03_end like BINARY '%{$que_str}%' /* 中餐送餐日-迄-MEMO(非熟代) */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds04_begin like BINARY '%{$que_str}%' /* 晚餐送餐日-起-MEMO(非熟代) */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds04_end like BINARY '%{$que_str}%' /* 晚餐送餐日-迄-MEMO(非熟代) */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds05_begin like BINARY '%{$que_str}%' /* 熟代送餐日-起 */                       
                        or BINARY {$tbl_meal_replacement_day_set}.mlds05_end like BINARY '%{$que_str}%' /* 熟代送餐日-迄 */                       
                        or {$tbl_meal_replacement_day_set}.mlds99 like '%{$que_str}%' /* 備註 */
                      )
                ";
    }

    if(!empty($get_data['que_mlds01'])) { // 送餐時間調整-起
      $que_mlds01 = $get_data['que_mlds01'];
      $que_mlds01 = $this->db->escape_like_str($que_mlds01);
      $where .= " and {$tbl_meal_replacement_day_set}.mlds01 = '{$que_mlds01}'  /* 送餐時間調整-起 */ ";
    }
    if(!empty($get_data['que_mlds02'])) { // 送餐時間調整-迄
      $que_mlds02 = $get_data['que_mlds02'];
      $que_mlds02 = $this->db->escape_like_str($que_mlds02);
      $where .= " and {$tbl_meal_replacement_day_set}.mlds02 = '{$que_mlds02}'  /* 送餐時間調整-迄 */ ";
    }
    ...
    // 計算查詢條件後的總筆數
    $sql_cnt = "select {$tbl_meal_replacement_day_set}.s_num
                from {$tbl_meal_replacement_day_set}
                where $where
                group by {$tbl_meal_replacement_day_set}.s_num
                order by {$tbl_meal_replacement_day_set}.s_num
               ";
    //u_var_dump($sql_cnt);
  ...
  public function save_add() {
    ...
    if(!$this->db->insert($tbl_meal_replacement_day_set, $post_data)) {
  ...
  public function save_upd() {
    ...
    if(!$this->db->update($tbl_meal_replacement_day_set, $post_data)) {
  ...
  public function save_is_available() {
    ...
    if(!$this->db->update($tbl_meal_replacement_day_set, $data)) {
      $rtn_msg = $this->lang->line('upd_ng'); // 修改失敗!!
    }
    echo $rtn_msg;
    return;
  ...
  private function _aes_fd() {
    ...
    foreach ($this->aes_fd as $k_fd_name => $v_fd_name) {
      $aes_fd .= ",AES_DECRYPT({$tbl_meal_replacement_day_set}.{$v_fd_name},'{$this->db_crypt_key2}') as {$v_fd_name}\n";
    }
    return($aes_fd);
  }
  // **************************************************************************
