...
class Donate1_que extends CI_Controller {
  public function __construct() {
    ...
    $this->tpl->assign('tv_return_link',front_url()."donate1_que/"); // return 預設到瀏覽畫面
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($donate_import_row,$row_cnt) = $this->donate_import_model->get_que_front($q_str,$pg); // 列出捐款資料
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_que_link',front_url()."donate1_que/que/{$q_str}");
    ...
    $config['base_url'] = front_url()."donate1_que/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = front_url()."/donate1_que/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("front/donate1_que.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: que()
  ...
  public function que($q_str)	{
    ...
    ////redirect(front_url()."donate1_que/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
     redirect(front_url()."donate1_que/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
