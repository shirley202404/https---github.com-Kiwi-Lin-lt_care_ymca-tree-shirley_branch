...
class Donate_progress extends CI_Controller {
  public function __construct() {
    ...
    $this->tpl->assign('tv_menu_title','捐款進度資料');
    ...
    $this->tpl->assign('tv_add_link',be_url().'donate_progress/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."donate_progress/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/donate_progress_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($donate_progress_row,$row_cnt) = $this->donate_progress_model->get_que($q_str,$pg); // 列出捐款進度資料
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','捐款進度資料瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'donate_progress/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'donate_progress/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'donate_progress/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'donate_progress/upd/');
    $this->tpl->assign('tv_del_link',be_url().'donate_progress/del/');
    $this->tpl->assign('tv_prn_link',be_url().'donate_progress/prn/');
    $this->tpl->assign('tv_que_link',be_url()."donate_progress/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'donate_progress/save/');
    $this->tpl->assign('tv_donate_progress_row',$donate_progress_row);
    $config['base_url'] = be_url()."donate_progress/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/donate_progress/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/donate_progress.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $donate_progress_row = $this->donate_progress_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($donate_progress_row);
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_donate_progress_row',$donate_progress_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."donate_progress/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."donate_progress/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."donate_progress/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'donate_progress/upd/');
    $this->tpl->display("be/donate_progress_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    ...
    $this->tpl->assign('tv_exit_link',be_url()."donate_progress/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."donate_progress/"); // 上一層連結位置
    $this->tpl->display("be/donate_progress_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    $msel = 'cpy';
    $donate_progress_row = $this->donate_progress_model->get_one($s_num);
    //u_var_dump($donate_progress_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->donate_progress_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'donate_progress/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->donate_progress_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->donate_progress_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->donate_progress_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."donate_progress/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."donate_progress/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  function __destruct() {
    ...
    $url_str[] = 'be/donate_progress/save';
    $url_str[] = 'be/donate_progress/del';
