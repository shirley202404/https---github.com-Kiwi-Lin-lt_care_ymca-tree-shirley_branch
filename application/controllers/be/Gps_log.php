...
class Gps_log extends CI_Controller {
  public function __construct() {
    parent::__construct();
    $this->zi_init->header('be'); // 網頁 head
    $this->load->model('route_model'); // 路徑資料
    $this->load->model('gps_log_model'); // GPS記錄檔
    ...
    $this->tpl->assign('tv_menu_title','GPS記錄檔');
    ...
    $this->tpl->assign('tv_add_link',be_url().'gps_log/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."gps_log/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/gps_log_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($gps_log_row,$row_cnt) = $this->gps_log_model->get_que($q_str,$pg); // 列出GPS記錄檔
    $route_h_row = $this->route_model->get_one($reh_s_num); // 列出單筆明細資料
    $route_sw_row = $this->route_model->get_route_sw($reh_s_num); // 路線規劃-輔助社工
    $dp_arr[$route_h_row->dp_s_num]["dp_s_num"] = $route_h_row->dp_s_num; 
    $dp_arr[$route_h_row->dp_s_num]["dp_name"] = $route_h_row->dp_name;
    if(NULL != $route_sw_row) {
      foreach ($route_sw_row as $k => $v) {
        $dp_arr[$k]["dp_s_num"] = $v['dp_s_num'];
        $dp_arr[$k]["dp_name"] ="{$v['dp01']}{$v['dp02']}";
      }
    }
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_reh_s_num',$reh_s_num);
    $this->tpl->assign('tv_title','GPS記錄檔瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'gps_log/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'gps_log/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'gps_log/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'gps_log/upd/');
    $this->tpl->assign('tv_del_link',be_url().'gps_log/del/');
    $this->tpl->assign('tv_prn_link',be_url().'gps_log/prn/');
    $this->tpl->assign('tv_route_history_link',be_url().'gps_log/route_history/');
    $this->tpl->assign('tv_que_link',be_url()."gps_log/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'gps_log/save/');
    $this->tpl->assign('tv_gps_log_row',$gps_log_row);
    $this->tpl->assign('tv_delivery_person_row',$dp_arr);
    $config['base_url'] = be_url()."gps_log/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/gps_log/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/gps_log.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $gps_log_row = $this->gps_log_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($gps_log_row);
    ...
    $this->tpl->assign('tv_upd_link',be_url().'gps_log/upd/');
    $this->tpl->display("be/gps_log_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add() { // 新增
    ...
    $this->tpl->assign('tv_exit_link',be_url()."gps_log/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."gps_log/"); // 上一層連結位置
    $this->tpl->display("be/gps_log_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num) {
    $msel = 'cpy';
    $gps_log_row = $this->gps_log_model->get_one($s_num);
    //u_var_dump($gps_log_row);
  ...
  public function del($s_num=NULL)  {
    $rtn_msg = $this->gps_log_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'gps_log/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)  {
    switch($kind) {
      case "add":
      case "cpy":
        $this->gps_log_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->gps_log_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->gps_log_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str) {
    ...
    ////redirect(be_url()."gps_log/p/1/q/{$q_str}", 'refresh');
    
    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."gps_log/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  ...
  function __destruct() {
    $url_str[] = 'be/gps_log/save';
    $url_str[] = 'be/gps_log/del';
