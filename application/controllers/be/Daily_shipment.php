...
class Daily_shipment extends CI_Controller {
  public function __construct() {
    ...
    $this->load->model('daily_shipment_model'); // 每日配送單資料
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    if($get_data['que_reh_s_num'] != NULL) {
      $dp_row = $this->delivery_person_model->get_all_is_available(); // 取得所有送餐員名單
      list($daily_shipment_row,$row_cnt) = $this->daily_shipment_model->get_que($q_str,$pg); // 列出每日配送單資料
    }
    // u_var_dump($daily_shipment_row);
    $routes_h_row = $this->route_model->get_all(); // 路徑資料
    ...
    $this->tpl->assign('tv_save_change_link',be_url().'daily_shipment/save/change');
    ...
    $this->tpl->assign('tv_dp_row',$dp_row);
    $this->tpl->assign('tv_route_h_row',$routes_h_row);
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $daily_shipment_row = $this->daily_work_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($daily_shipment_row);
  ...
  public function cpy($s_num) {
    $msel = 'cpy';
    $daily_shipment_row = $this->daily_work_model->get_one($s_num);
    //u_var_dump($daily_shipment_row);
  ...
  public function del($s_num=NULL)  {
    $rtn_msg = $this->daily_work_model->del($s_num); // 刪除
    if($rtn_msg) {
  ...
  public function save($kind=NULL)  {
    switch($kind) {
      ...
      case "upd_this_page": // 本頁儲存
        ...
        break;
      case "change": // 本頁儲存
        // u_var_dump($this->input->post());
        // return;
        $rtn_msg = $this->daily_shipment_model->save_change(); // 路線調配
        if($rtn_msg == 'ok') {
          $this->zi_my_func->web_api_data("daily_shipment"); // 重新送一次資料到即時地圖
        }
        echo $rtn_msg;
        break;
    }
