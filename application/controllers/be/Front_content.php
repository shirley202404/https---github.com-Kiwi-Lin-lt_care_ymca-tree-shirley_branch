...
class Front_content extends CI_Controller {
  public function __construct() {
    parent::__construct();
    $this->zi_init->header('be'); // 網頁 head
    $this->load->model('front_content_model'); // 捐款前台內容
    ...
    $this->tpl->assign('tv_menu_title','捐款前台內容');
    ...
    $this->tpl->assign('tv_add_link',be_url().'front_content/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."front_content/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/front_content_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
    ...
    // 捐款區塊
    $this->fc01[30]["fc01"] = "30";
    $this->fc01[30]["active"] = "";
    $this->fc01[30]["fc01_str"] = "募款區塊";
    $this->fc01[30]["fc01_item"][1] = "捐款標題";
    $this->fc01[30]["fc01_item"][2] = "階段一文字";
    $this->fc01[30]["fc01_item"][3] = "階段二文字";
    $this->fc01[30]["fc01_item"][4] = "階段三文字";
    // 常見問題
    $this->fc01[35]["fc01"] = "35";
    $this->fc01[35]["active"] = "";
    $this->fc01[35]["fc01_str"] = "常見問題";
    $this->fc01[35]["fc01_item"][1] = "常見問題";
    // 頁尾
    $this->fc01[40]["fc01"] = "40";
    $this->fc01[40]["active"] = "";
    $this->fc01[40]["fc01_str"] = "頁尾";
    $this->fc01[40]["fc01_item"][1] = "衛服部字號";
    return;
  }
  // **************************************************************************
  //  函數名稱: index
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    if(isset($get_data['que_fc01'])) {
      $que_fc01 = $get_data['que_fc01'];
      $this->fc01[$que_fc01]['active'] = "active";
    }
    else {
      $this->fc01[30]['active'] = "active";
    }
    $que_fc01_str = "?que_fc01={$que_fc01}";
    list($front_content_row,$row_cnt) = $this->front_content_model->get_que($q_str,$pg); // 列出捐款前台內容
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','捐款前台內容瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url()."front_content/add/");
    $this->tpl->assign('tv_cpy_link',be_url()."front_content/cpy/");
    $this->tpl->assign('tv_disp_link',be_url()."front_content/disp/");
    $this->tpl->assign('tv_upd_link',be_url()."front_content/upd/");
    $this->tpl->assign('tv_del_link',be_url()."front_content/del/");
    $this->tpl->assign('tv_prn_link',be_url()."front_content/prn/");
    $this->tpl->assign('tv_que_link',be_url()."front_content/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'front_content/save/');
    $this->tpl->assign('tv_que_fc01_link',be_url().'front_content/?que_fc01=');
    $this->tpl->assign('tv_que_fc01',$que_fc01);
    $this->tpl->assign('tv_que_fc01_str',$que_fc01_str);
    $this->tpl->assign('tv_fc01_row',$this->fc01);
    $this->tpl->assign('tv_fc01_item_row',$this->fc01[$que_fc01]['fc01_item']);
    $this->tpl->assign('tv_front_content_row',$front_content_row);
    $config['base_url'] = be_url()."front_content/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/front_content/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/front_content.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $fc01_item_arr = NULL;
    if(isset($_GET['que_fc01'])) {
      $fc01_item_arr = $this->fc01[$_GET['que_fc01']]["fc01_item"];
    }
    $front_content_row = $this->front_content_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($front_content_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('add_ok')); // 新增成功!!
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_fc01',$_GET['que_fc01']);
    $this->tpl->assign('tv_front_content_row',$front_content_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."front_content/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."front_content/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."front_content/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'front_content/upd/');
    $this->tpl->display("be/front_content_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    ...
    $front_content_row = NULL;
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 新增
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_fc01',$_GET['que_fc01']);
    $this->tpl->assign('tv_fc01_item_arr',$fc01_item_arr);
    $this->tpl->assign('tv_save_ok',$this->lang->line('add_ok')); // 新增成功!!
    $this->tpl->assign('tv_front_content_row',$front_content_row);
    $this->tpl->assign('tv_save_link',be_url()."front_content/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."front_content/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."front_content/"); // 上一層連結位置
    $this->tpl->display("be/front_content_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    ...
    $front_content_row = $this->front_content_model->get_one($s_num);
    //u_var_dump($front_content_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->front_content_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'front_content/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->front_content_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->front_content_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->front_content_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."front_content/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."front_content/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  function __destruct() {
    ...
    $url_str[] = 'be/front_content/save';
    $url_str[] = 'be/front_content/del';
