...
class Region_setting extends CI_Controller {
  public function __construct() {
    ...
    $this->tpl->assign('tv_menu_title','區域設定檔');
    ...
    $this->tpl->assign('tv_add_link',be_url().'region_setting/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."region_setting/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/region_setting_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($region_setting_row,$row_cnt) = $this->region_setting_model->get_que($q_str,$pg); // 列出區域設定檔
    if(NULL != $region_setting_row) {
      foreach($region_setting_row as $k => $v) {
        $region_setting_row[$k]['rs04'] = str_replace(array("1", "2"), array("衛生局", "社會局"), $v['rs04']);
      }  
    }
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','區域設定檔瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'region_setting/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'region_setting/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'region_setting/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'region_setting/upd/');
    $this->tpl->assign('tv_del_link',be_url().'region_setting/del/');
    $this->tpl->assign('tv_prn_link',be_url().'region_setting/prn/');
    $this->tpl->assign('tv_que_link',be_url()."region_setting/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'region_setting/save/');
    $this->tpl->assign('tv_region_setting_row',$region_setting_row);
    $config['base_url'] = be_url()."region_setting/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/region_setting/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/region_setting.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $region_setting_row = $this->region_setting_model->get_one($s_num); // 列出單筆明細資料
    $region_setting_row->rs04 = str_replace(array("1", "2"), array("衛生局", "社會局"), $region_setting_row->rs04);
    //u_var_dump($region_setting_row);
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_region_setting_row',$region_setting_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."region_setting/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."region_setting/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."region_setting/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'region_setting/upd/');
    $this->tpl->display("be/region_setting_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    ...
    $this->tpl->assign('tv_exit_link',be_url()."region_setting/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."region_setting/"); // 上一層連結位置
    $this->tpl->display("be/region_setting_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    $msel = 'cpy';
    $region_setting_row = $this->region_setting_model->get_one($s_num);
    //u_var_dump($region_setting_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->region_setting_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'region_setting/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->region_setting_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->region_setting_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->region_setting_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."region_setting/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."region_setting/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  function __destruct() {
    ...
    $url_str[] = 'be/region_setting/save';
    $url_str[] = 'be/region_setting/del';
