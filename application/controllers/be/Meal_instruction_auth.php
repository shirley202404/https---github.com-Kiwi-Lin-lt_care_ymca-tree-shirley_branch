...
class Meal_instruction_auth extends CI_Controller {
  public function __construct() {
    ...
    $this->tpl->assign('tv_back_btn',$this->lang->line('back')); // 退回按鈕文字
    $this->tpl->assign('tv_add_link',be_url().'meal_instruction_auth/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."meal_instruction_auth/"); // return 預設到瀏覽畫面
    $this->tpl->assign('tv_upd_ok',$this->lang->line('upd_ok')); // 修改成功!!
    $this->tpl->assign('tv_add_ok',$this->lang->line('add_ok')); // 新增成功!!
    $this->tpl->assign('tv_back_ok',$this->lang->line('back_ok')); // 退回成功!!
    ...
    $this->tpl->assign('tv_back_ng',$this->lang->line('back_ng')); // 退回失敗!!
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/meal_instruction_auth_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($meal_instruction_auth_row,$row_cnt) = $this->meal_instruction_auth_model->get_que($q_str,$pg); // 列出異動單審核紀錄檔
    // u_var_dump($meal_instruction_auth_row);
    ...
    $this->tpl->assign('tv_add_link',be_url().'meal_instruction_auth/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'meal_instruction_auth/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'meal_instruction_auth/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'meal_instruction_auth/upd/');
    $this->tpl->assign('tv_del_link',be_url().'meal_instruction_auth/del/');
    $this->tpl->assign('tv_back_link',be_url().'meal_instruction_auth/back/');
    $this->tpl->assign('tv_prn_link',be_url().'meal_instruction_auth/prn/');
    $this->tpl->assign('tv_que_link',be_url()."meal_instruction_auth/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'meal_instruction_auth/save/');
    $this->tpl->assign('tv_sw_auth_link',be_url().'meal_instruction_auth/sw_auth/'); // 社工確認
    $this->tpl->assign('tv_wo_auth_link',be_url().'meal_instruction_auth/wo_auth/'); // 核銷人員確認
    $this->tpl->assign('tv_me_auth_link',be_url().'meal_instruction_auth/me_auth/'); // 製餐確認
    $this->tpl->assign('tv_checked_link',be_url().'meal_instruction_auth/?que_check_type=checked');
    $this->tpl->assign('tv_unchecked_link',be_url().'meal_instruction_auth/');
    $this->tpl->assign('tv_meal_instruction_auth_row',$meal_instruction_auth_row);
    $this->tpl->assign('tv_action',$action);
    $this->tpl->assign('tv_is_check',$is_check);
    $config['base_url'] = be_url()."meal_instruction_auth/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/meal_instruction_auth/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/meal_instruction_auth.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $meal_instruction_auth_row = $this->meal_instruction_auth_model->get_one($s_num); // 列出單筆明細資料
    $mil_h_s_num = $meal_instruction_auth_row->mil_s_num;
    $meal_instruction_log_h_row = $this->meal_instruction_log_h_model->get_one($mil_h_s_num); // 列出單筆明細資料
    $mil02_arr = explode(",", $meal_instruction_log_h_row->mil02);
    $mil02_str_arr = NULL;
    $meal_instruction_log_m_row = NULL;
    $meal_instruction_log_mp_row = NULL;
    $meal_instruction_log_s_row = NULL;
    $meal_instruction_log_p_row = NULL;
    ...
    foreach ($mil02_arr as $v) {
      switch ($v) {  
        ...
        break;
        case 6: // 補班日出餐
          $meal_instruction_log_d_row = $this->meal_instruction_log_h_model->get_d_by_s_num($mil_h_s_num); // 列出補班日出餐資料
          if(NULL != $meal_instruction_log_d_row) {
            $mil02_str_arr[] = "補班日出餐";
          }
        break;  
      } 
      $meal_instruction_log_h_row->mil02_str = join("、", $mil02_str_arr);
    }
    // u_var_dump($meal_instruction_auth_row);
    ...
    $this->tpl->assign('tv_save_link',be_url()."meal_instruction_auth/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."meal_instruction_auth/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."meal_instruction_auth/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'meal_instruction_auth/upd/');
    $this->tpl->display("be/meal_instruction_auth_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add() { // 新增
    $msel = 'add';
    $meal_instruction_auth_row = NULL;
    ...
    $this->tpl->assign('tv_exit_link',be_url()."meal_instruction_auth/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."meal_instruction_auth/"); // 上一層連結位置
    $this->tpl->display("be/meal_instruction_auth_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num) {
    $msel = 'cpy';
    $meal_instruction_auth_row = $this->meal_instruction_auth_model->get_one($s_num);
    //u_var_dump($meal_instruction_auth_row);
  ...
  public function del($s_num=NULL)  {
    $rtn_msg = $this->meal_instruction_auth_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'meal_instruction_auth/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  //  設計日期: 2021-01-04
  // **************************************************************************
  public function back($s_num=NULL)  {
    $rtn_msg = $this->meal_instruction_auth_model->back($s_num); // 退回
    if($rtn_msg) {
      redirect(be_url().'meal_instruction_auth/', 'refresh');
    }
    else {
  ...
  public function save($kind=NULL)  {
    switch($kind) {
      case "add":
      case "cpy":
        $this->meal_instruction_auth_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->meal_instruction_auth_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->meal_instruction_auth_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str) {
    ...
    ////redirect(be_url()."meal_instruction_auth/p/1/q/{$q_str}", 'refresh');
    
    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."meal_instruction_auth/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  ...
  public function sw_auth($s_num)  {
    $rtn_msg = $this->meal_instruction_auth_model->save_sw_auth($s_num); // 確定
  ...
  public function wo_auth($s_num)  {
    $rtn_msg = $this->meal_instruction_auth_model->save_wo_auth($s_num); // 確定
    $this->chk_auth_times($s_num);
    header('location: '.$_SERVER['HTTP_REFERER']);
    return;
  }
  ...
  public function me_auth($s_num)  {
    $rtn_msg = $this->meal_instruction_auth_model->save_me_auth($s_num); // 確定
    $this->chk_auth_times($s_num);
    header('location: '.$_SERVER['HTTP_REFERER']);
    return;
  }
  ...
  // **************************************************************************
  //  函數名稱: chk_auth_times()
  //  函數功能: 只需社工核准(20210610 MARK BY Kiwi)
  //  程式設計: Kiwi
  //  設計日期: 2021/01/02
  // **************************************************************************
  public function chk_auth_times($s_num)  {
    $chk_row = $this->meal_instruction_auth_model->get_one($s_num); // 確定
    $mil_s_num = $chk_row->mil_s_num;
    $sec_s_num = $chk_row->sec_s_num;
    $this->meal_instruction_log_h_model->upd_mil03_by_s_num($mil_s_num);
    return;
  }
  
  function __destruct() {
    $url_str[] = 'be/meal_instruction_auth/save';
    $url_str[] = 'be/meal_instruction_auth/del';
