...
class Daily_production_order extends CI_Controller {
  public function __construct() {
    parent::__construct();
    $this->zi_init->header('be'); // 網頁 head
    $this->load->model('meal_model'); // 餐點資料
    $this->load->model('route_model'); // 路徑資料
    $this->load->model('daily_production_order_model'); // 餐條排序設定檔
    ...
    $this->tpl->assign('tv_menu_title','餐條排序設定檔');
    ...
    $this->tpl->assign('tv_add_link',be_url().'daily_production_order/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."daily_production_order/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/daily_production_order_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($daily_production_order_row,$row_cnt) = $this->daily_production_order_model->get_que($q_str,$pg); // 列出餐條排序設定檔
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','餐條排序設定檔瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'daily_production_order/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'daily_production_order/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'daily_production_order/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'daily_production_order/upd/');
    $this->tpl->assign('tv_del_link',be_url().'daily_production_order/del/');
    $this->tpl->assign('tv_prn_link',be_url().'daily_production_order/prn/');
    $this->tpl->assign('tv_que_link',be_url()."daily_production_order/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'daily_production_order/save/');
    $this->tpl->assign('tv_daily_production_order_row',$daily_production_order_row);
    $config['base_url'] = be_url()."daily_production_order/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/daily_production_order/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/daily_production_order.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $daily_production_order_row = $this->daily_production_order_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($daily_production_order_row);
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_daily_production_order_row',$daily_production_order_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."daily_production_order/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."daily_production_order/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."daily_production_order/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'daily_production_order/upd/');
    $this->tpl->display("be/daily_production_order_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    ...
    $route_row = $this->route_model->get_all(); // 路徑資料
    $daily_production_order_row = NULL;
    $daily_production_order_row['dpo04'] = array();
    $daily_production_order_row['dpo05'] = array();
    $daily_production_order_row['dpo06'] = array();
    $daily_production_order_row['dpo07'] = array();
    $daily_production_order_row['dpo08'] = array();
    $daily_production_order_row['dpo09'] = array();
    $daily_production_order_row['dpo10'] = array();
    ...
    $this->tpl->assign('tv_daily_production_order_row', (object) $daily_production_order_row);
    $this->tpl->assign('tv_save_link',be_url()."daily_production_order/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."daily_production_order/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."daily_production_order/"); // 上一層連結位置
    $this->tpl->display("be/daily_production_order_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    ...
    $route_row = $this->route_model->get_all(); // 路徑資料
    $meals_row = $this->meal_model->get_all(); // 餐點 
    $daily_production_order_row = $this->daily_production_order_model->get_one($s_num);
    $chk_arr = array('dpo04', 'dpo05', 'dpo06', 'dpo07', 'dpo08', 'dpo09', 'dpo10');
    foreach ($chk_arr as $k => $v) {
      if($daily_production_order_row->$v == NULL) {
        $daily_production_order_row->$v = array();
      }
    }
    //u_var_dump($daily_production_order_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->daily_production_order_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'daily_production_order/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->daily_production_order_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->daily_production_order_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->daily_production_order_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."daily_production_order/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."daily_production_order/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  function __destruct() {
    ...
    $url_str[] = 'be/daily_production_order/save';
    $url_str[] = 'be/daily_production_order/del';
