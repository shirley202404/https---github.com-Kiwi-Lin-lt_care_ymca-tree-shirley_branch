...
class Verification_person extends CI_Controller {
  public function __construct() {
    ...
    $this->tpl->assign('tv_menu_title','核銷人員資料檔');
    ...
    $this->tpl->assign('tv_add_link',be_url().'verification_person/add/');
    $this->tpl->assign('tv_pdf_flag','Y'); // 是否顯示pdf按鈕
    $this->tpl->assign('tv_pdf_btn',$this->lang->line('pdf')); // 輸出pdf按鈕文字
    $this->tpl->assign('tv_download_execl_flag','Y'); // 是否顯示下載execl按鈕
    $this->tpl->assign('tv_download_execl_btn',$this->lang->line('download_execl')); // 下載execl按鈕文字
    $this->tpl->assign('tv_return_link',be_url()."verification_person/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/verification_person_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($verification_person_row,$row_cnt) = $this->verification_person_model->get_que($q_str,$pg); // 列出核銷人員資料檔
    if(NULL != $verification_person_row) {
      foreach ($verification_person_row as $k => $v) {
        $ch_file_name_en = base64url_encode("{$v['vp11']}");
        $en_file_name_en = base64url_encode("{$v['vp11']}");
        $verification_person_row[$k]['download_url'] = be_url()."verification_person/download_file/{$ch_file_name_en}/{$en_file_name_en}";
      }
    }
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','核銷人員資料檔瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'verification_person/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'verification_person/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'verification_person/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'verification_person/upd/');
    $this->tpl->assign('tv_del_link',be_url().'verification_person/del/');
    $this->tpl->assign('tv_prn_link',be_url().'verification_person/prn/');
    $this->tpl->assign('tv_que_link',be_url()."verification_person/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'verification_person/save/');
    $this->tpl->assign('tv_verification_person_row',$verification_person_row);
    $config['base_url'] = be_url()."verification_person/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/verification_person/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/verification_person.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $verification_person_row = $this->verification_person_model->get_one($s_num); // 列出單筆明細資料
    if(NULL != $verification_person_row->vp11) {
      $ch_file_name_en = base64url_encode("{$verification_person_row->vp11}");
      $en_file_name_en = base64url_encode("{$verification_person_row->vp11}");
      $verification_person_row->download_url = be_url()."verification_person/download_file/{$ch_file_name_en}/{$en_file_name_en}";
    }
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_verification_person_row',$verification_person_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."verification_person/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."verification_person/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."verification_person/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'verification_person/upd/');
    $this->tpl->display("be/verification_person_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    ...
    $this->tpl->assign('tv_exit_link',be_url()."verification_person/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."verification_person/"); // 上一層連結位置
    $this->tpl->display("be/verification_person_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    $msel = 'cpy';
    $verification_person_row = $this->verification_person_model->get_one($s_num);
    //u_var_dump($verification_person_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->verification_person_model->del($s_num); // 刪除
    if($rtn_msg) {
      redirect(be_url().'verification_person/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->verification_person_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->verification_person_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->verification_person_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."verification_person/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."verification_person/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  public function download_file($download_file_name, $save_file_name) {    
    ...
    $path = FCPATH."upload_files/verification_person/";
  ...
  function __destruct() {
    ...
    $url_str[] = 'be/verification_person/save';
    $url_str[] = 'be/verification_person/del';
