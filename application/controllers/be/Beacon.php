...
class Beacon extends CI_Controller {
  public function __construct() {
    ...
    $this->load->model('sys_language_model'); // 語系
	  ...
	  $this->tpl->assign('tv_menu_title','Beacon資料');
    ...
    $this->tpl->assign('tv_disp_btn',$this->lang->line('disp')); // 明細按鈕文字
	  ...
	  $this->tpl->assign('tv_add_link',be_url().'beacon/add/');
	  ...
	  $this->tpl->assign('tv_return_link',be_url()."beacon/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/beacon_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
  ...
  // **************************************************************************
    ...
    list($beacon_row,$row_cnt) = $this->beacon_model->get_que($q_str,$pg); // 列出Beacon資料
	  $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
	  $this->tpl->assign('tv_msel',$msel);
	  $this->tpl->assign('tv_title','Beacon資料瀏覽');
	  $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'beacon/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'beacon/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'beacon/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'beacon/upd/');
    $this->tpl->assign('tv_del_link',be_url().'beacon/del/');
    $this->tpl->assign('tv_prn_link',be_url().'beacon/prn/');
    $this->tpl->assign('tv_que_link',be_url()."beacon/que/{$q_str}");
    ...
    $this->tpl->assign('tv_que_order_kind',$_SESSION[$q_str]['que_order_kind']); // 排序類別
	  $this->tpl->assign('tv_save_link',be_url().'beacon/save/');
	  $this->tpl->assign('tv_beacon_row',$beacon_row);
    $config['base_url'] = be_url()."beacon/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/beacon/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->assign('tv_total_rows',$row_cnt);
	  ...
	  $this->tpl->display("be/beacon.html");
	  return;
	}
  // **************************************************************************
  //  函數名稱: disp()
  ...
  // **************************************************************************
	public function disp($s_num) {
	  $msel = 'disp';
    $beacon_row = $this->beacon_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($beacon_row);
	  $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
	  $this->tpl->assign('tv_msel',$msel);
	  $this->tpl->assign('tv_beacon_row',$beacon_row);
	  $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
	  $this->tpl->assign('tv_save_link',be_url()."beacon/save/{$msel}");
	  $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
	  $this->tpl->assign('tv_exit_link',be_url()."beacon/"); // 離開按鈕的連結位置
	  $this->tpl->assign('tv_parent_link',be_url()."beacon/"); // 上一層連結位置
	  $this->tpl->assign('tv_upd_link',be_url().'beacon/upd/');
    $this->tpl->display("be/beacon_disp.html");
	  return;
	}
  // **************************************************************************
  //  函數名稱: add()
  ...
  // **************************************************************************
	public function add()	{ // 新增
	  ...
	  $this->tpl->assign('tv_exit_link',be_url()."beacon/"); // 離開按鈕的連結位置
	  $this->tpl->assign('tv_parent_link',be_url()."beacon/"); // 上一層連結位置
	  $this->tpl->display("be/beacon_edit.html");
	  return;
	}
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  // **************************************************************************
	public function cpy($s_num)	{
	  $msel = 'cpy';
    $beacon_row = $this->beacon_model->get_one($s_num);
    //u_var_dump($beacon_row);
  ...
  // **************************************************************************
	public function del($s_num=NULL)	{
	  $rtn_msg = $this->beacon_model->del($s_num); // 刪除
	  if($rtn_msg) {
	    redirect(be_url().'beacon/', 'refresh');
	  }
	  else {
	    die($rtn_msg); // 刪除失敗!!!
	  }
  ...
  // **************************************************************************
	public function save($kind=NULL)	{
	  switch($kind) {
	    case "add":
	    case "cpy":
	      $this->beacon_model->save_add(); // 新增儲存
	      break;
	    case "upd":
	      $this->beacon_model->save_upd(); // 修改儲存
	      break;
	    case "upd_is_available":
	      $this->beacon_model->save_is_available(); // 上下架儲存
	      break;
	  }
	  return;
	}
  ...
  // **************************************************************************
    ...
    ////$_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
	  ////redirect(be_url()."beacon/p/1/q/{$q_str}", 'refresh');
	  
	  // GET 使用
	  $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
	  redirect(be_url()."beacon/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
	  return;
  }
  ...
  function __destruct() {
    $url_str[] = 'be/beacon/save';
    $url_str[] = 'be/beacon/del';
