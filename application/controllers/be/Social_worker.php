...
class Social_worker extends CI_Controller {
  public function __construct() {
    ...
    $this->load->model('sys_account_group_model'); // 管理群組
    ...
    $this->tpl->assign('tv_menu_title','社工基本資料檔');
    ...
    $this->tpl->assign('tv_add_link',be_url().'social_worker/add/');
    ...
    $this->tpl->assign('tv_return_link',be_url()."social_worker/"); // return 預設到瀏覽畫面
    ...
    $this->tpl->assign('tv_upload_link', be_url() . 'fileupload/upload/social_worker_/'); // upload/"origin" 保留上傳檔名，測試機要注意中文的問題
    $this->tpl->assign('tv_upload_path', pub_url('') . 'uploads/files/');
    $this->tpl->assign('tv_chk_sw03_link', be_url()."social_worker/chk_sw03");
  ...
  public function index($pg=1,$q_str=NULL) {
    ...
    list($social_worker_row,$row_cnt) = $this->social_worker_model->get_que($q_str,$pg); // 列出社工基本資料檔
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 瀏覽
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_title','社工基本資料檔瀏覽');
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_add_link',be_url().'social_worker/add/');
    $this->tpl->assign('tv_cpy_link',be_url().'social_worker/cpy/');
    $this->tpl->assign('tv_disp_link',be_url().'social_worker/disp/');
    $this->tpl->assign('tv_upd_link',be_url().'social_worker/upd/');
    $this->tpl->assign('tv_del_link',be_url().'social_worker/del/');
    $this->tpl->assign('tv_prn_link',be_url().'social_worker/prn/');
    $this->tpl->assign('tv_que_link',be_url()."social_worker/que/{$q_str}");
    ...
    $this->tpl->assign('tv_save_link',be_url().'social_worker/save/');
    $this->tpl->assign('tv_social_worker_row',$social_worker_row);
    $config['base_url'] = be_url()."social_worker/p/";
    $config['suffix'] = "/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    $config['first_url'] = be_url()."/social_worker/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}";
    ...
    $this->tpl->display("be/social_worker.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: disp()
  ...
  public function disp($s_num) {
    $msel = 'disp';
    $sw_ra_row = $this->social_worker_model->get_ra($s_num);
    $social_worker_row = $this->social_worker_model->get_one($s_num); // 列出單筆明細資料
    //u_var_dump($social_worker_row);
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 明細
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_sw_ra_row',$sw_ra_row);
    $this->tpl->assign('tv_social_worker_row',$social_worker_row);
    $this->tpl->assign('tv_save_ok',$this->lang->line('upd_ok')); // 更新成功!!
    $this->tpl->assign('tv_save_link',be_url()."social_worker/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."social_worker/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."social_worker/"); // 上一層連結位置
    $this->tpl->assign('tv_upd_link',be_url().'social_worker/upd/');
    $this->tpl->display("be/social_worker_disp.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: add()
  ...
  public function add()	{ // 新增
    $msel = 'add';
    $sw_ra_row = array();
    $social_worker_row = NULL;
    $sys_account_group_row = $this->sys_account_group_model->get_all_is_open();
    $this->tpl->assign('tv_breadcrumb3',$this->lang->line($msel)); // 新增
    $this->tpl->assign('tv_msel',$msel);
    $this->tpl->assign('tv_save_ok',$this->lang->line('add_ok')); // 新增成功!!
    $this->tpl->assign('tv_sw_ra_row',$sw_ra_row);
    $this->tpl->assign('tv_social_worker_row',$social_worker_row);
    $this->tpl->assign('tv_sys_account_group_row',$sys_account_group_row);
    $this->tpl->assign('tv_save_link',be_url()."social_worker/save/{$msel}");
    $this->tpl->assign('tv_now_link',base_url().uri_string()); // 目前網址
    $this->tpl->assign('tv_exit_link',be_url()."social_worker/"); // 離開按鈕的連結位置
    $this->tpl->assign('tv_parent_link',be_url()."social_worker/"); // 上一層連結位置
    $this->tpl->display("be/social_worker_edit.html");
    return;
  }
  // **************************************************************************
  //  函數名稱: cpy()
  ...
  public function cpy($s_num)	{
    $msel = 'cpy';
    $sw_ra_row = (array) $this->social_worker_model->get_ra($s_num);
    $social_worker_row = $this->social_worker_model->get_one($s_num);
    $sys_account_group_row = $this->sys_account_group_model->get_all_is_open();
    //u_var_dump($social_worker_row);
  ...
  public function del($s_num=NULL)	{
    $rtn_msg = $this->social_worker_model->del($s_num); // 刪除
    $this->zi_my_func->web_api_data("delivery_person", "del");
    if($rtn_msg) {
      redirect(be_url().'social_worker/', 'refresh');
    }
    else {
      die($rtn_msg); // 刪除失敗!!!
    }
  ...
  public function save($kind=NULL)	{
    switch($kind) {
      case "add":
      case "cpy":
        $this->social_worker_model->save_add(); // 新增儲存
        break;
      case "upd":
        $this->social_worker_model->save_upd(); // 修改儲存
        break;
      case "upd_is_available":
        $this->social_worker_model->save_is_available(); // 上下架儲存
        break;
    }
    return;
  }
  ...
  public function que($q_str)	{
    ...
    ////redirect(be_url()."social_worker/p/1/q/{$q_str}", 'refresh');

    // GET 使用
    $data = $this->input->get(); // GET 用
    $_SESSION[$q_str]['que_order_fd_name'] = $data['que_order_fd_name']; // 排序欄位
    $_SESSION[$q_str]['que_order_kind'] = $data['que_order_kind']; // 排序類別
    redirect(be_url()."social_worker/p/1/q/{$q_str}/?{$_SERVER['QUERY_STRING']}", 'refresh');
    return;
  }
  // **************************************************************************
  //  函數名稱: _que_start()
  ...
  public function chk_sw03()	{
    $socail_worker_row = $this->social_worker_model->chk_sw03();
    if($socail_worker_row != NULL) {
      echo "false";
      return;
    }
    echo "true";
  ...
  function __destruct() {
    $url_str[] = 'be/social_worker/save';
    $url_str[] = 'be/social_worker/del';
    $url_str[] = 'be/social_worker/chk_sw03';
