...
class UploadHandler
{
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error,
            $index, $content_range) {
        // Add by Tony 2018/6/4
        if('origin' == $this->options['std_name']) { // 使用原本檔名
          return ($name);
        }
        // Add by Tony 變亂數,避免中文的問題 Begin //
        ...
        $name = $this->options['std_name'] . $randomString . '.' . strtolower(substr(strrchr($name, '.'), 1));  // 使用自訂
        //$name = date('Y-m-d H:i:s') . '-' . $randomString . '.' . strtolower(substr(strrchr($name, '.'), 1));  // 使用日期
        return ($name);
        // Add by Tony 變亂數,避免中文的問題 End //
